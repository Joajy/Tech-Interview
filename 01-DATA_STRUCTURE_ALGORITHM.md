...
<li> Big-O, Big-Theta, Big-Omega 에 대해 설명해 주세요.</li>
  -> Big-O: 알고리즘 효율성의 상한선(최악 케이스 고려)<br>
  -> Big-Theta: 알고리즘 효율선의 상한선과 하한선 사이(평균 케이스)<br>
  -> Big-Omega: 알고리즘 효율성의 하한선(최적 케이스)<br>
<li> 다른 것을 사용하지 않고, Big-O를 사용하는 이유가 있을까요?</li>
    -> <strong> 알고리즘 효율성의 상한선을 나타내므로, 최악의 케이스에 대해 대처할 수 있기 때문입니다.</strong>
      (참고) Big-Theta(상한선과 하한선 사이), Big-Omega(하한선)
<li> O(1)은 O(N^2) 보다 무조건적으로 빠른가요?</li>
    -> <strong>말 그대로 상한선에 대한 표기이기 때문에, 실제 어떠한 케이스가 주어지느냐에 따라 달라질 수 있습니다. 다만, 대부분의 케이스에서는 맞는 정론이라 볼 수 있겠습니다.</strong>
</ul>
</details>
