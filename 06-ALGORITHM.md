...
12. -> Divide And Conquer(분할 정복)으로 처리할 수 있을 것 같다.
13. 정렬된 두 LinkedList를 합쳐 하나의 정렬된 LinkedList로 만드는 코드를 작성해 보세요.
14. 어떤 수가 주어졌을 때, 이 수의 소인수를 모두 구하는 코드를 작성해 보세요.
15. 1 부터 1,000,000 까지의 수를 이어 붙인다고 가정할 때, 이 수에서 0이 몇 번이나 등장하는지 확인하는 코드를 작성해 보세요.
16. 스택 두개로 큐를, 큐 두개로 스택을 구현하는 코드를 작성해 보세요.
